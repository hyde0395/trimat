...
def test_pack_weights_absmean_preserves_outlier_signal():
    ...
    # Values away from the 0.5 boundary so the result is rounding-rule-independent.
    ...
    # absmax: codes=[1,0,0,0], scale=10 -> 10 ; absmean: codes=[1,1,1,1], scale=4.75 -> 19
    np.testing.assert_allclose(trimat.gemv(out_max, x)[0], 10.0, atol=1e-4)
    np.testing.assert_allclose(trimat.gemv(out_mean, x)[0], 19.0, atol=1e-4)
...
def test_load_safetensors_absmean(tmp_path):
    ...
    x = np.ones(4, dtype=np.float32)
    np.testing.assert_allclose(trimat.gemv(loaded["layer.weight"], x)[0], 19.0, atol=1e-4)
