    ...
    *,
    ...
    for name, value in state_dict.items():
        ...
        if _is_packable(arr) and (should_pack is None or should_pack(name)):
            out[name] = pack(np.ascontiguousarray(arr, dtype=np.float32))
        else:
            out[name] = arr
    return out
...
def load_safetensors(
    ...
    *,
    ...
    Returns a ``name -> TernaryTensor | ndarray`` mapping.
    """
    ...
    return pack_weights(state, should_pack=should_pack)
...
def from_pretrained(
    ...
    *,
    ...
    return load_safetensors(local_path, should_pack=should_pack)
